...
// --- БЛОК РЕГИСТРАЦИИ ПАРАМЕТРОВ С ХЕШ-КОММЕНТАРИЯМИ ДЛЯ PARSER_XML ---
// @Group: STEER_
...
const AP_Param::GroupInfo AP_ModbusSteering::var_info[] = {
    ...
    // @Description: Максимальный рабочий диапазон руля в импульсах
    // @User: Standard
    AP_GROUPINFO("MAX_STEPS", 3, AP_ModbusSteering, max_steps, 2000),
    
    // @Param: START_SPD
    // @DisplayName: Start JOG Speed
    // @Description: Стартовая скорость JOG (Регистр 0x0030)
    // @User: Standard
...
void AP_ModbusSteering::update(float steering_out) {
    ...
    // --- РАСШИРЕННЫЙ ОТЛАДОЧНЫЙ ВЫВОД В КОНСОЛЬ ---
    static uint32_t last_debug_steer_ms = 0;
    if (now - last_debug_steer_ms >= 2000) {
        last_debug_steer_ms = now;
        
        const char* state_str = "UNKNOWN";
        switch (static_cast<int>(current_state)) {
            case 0: state_str = "INIT_ENABLE"; break;
            case 1: state_str = "INIT_SUBDIVISION"; break;
            case 2: state_str = "INIT_START_SPD"; break;
            case 3: state_str = "INIT_MAX_SPD"; break;
            case 4: state_str = "INIT_TIMEOUT"; break;
            case 5: state_str = "INIT_ACCEL"; break;
            case 6: state_str = "INIT_DECEL"; break;
            case 7: state_str = "RUN_WRITE_POS"; break;
            case 8: state_str = "RUN_READ_TELEMETRY"; break;
        }
        
        gcs().send_text(MAV_SEVERITY_INFO, "CL57R: State=%s | InputSteer=%.3f", state_str, (double)steering_out);
    }
    // -------------------------------------------------

    // --- БЛОК ПАРСИНГА ОТВЕТОВ ВНУТРИ C++ ---
    if (available_bytes > 0) {
        ...
        // 1. Парсинг эха для шагов инициализации
            ...
            for (uint32_t i = 0; i <= available_bytes - 8; i++) {
                if (local_buf[i] == (uint8_t)slave_id.get()) {
                    ...
                    if (modbus_crc16(&local_buf[i], 6) == received_crc) {
                        if (current_state == DriveState::INIT_ENABLE && reg == 0x0038) response_received = true;
                        if (current_state == DriveState::INIT_SUBDIVISION && reg == 0x0023) response_received = true;
                        if (current_state == DriveState::INIT_START_SPD && reg == 0x0030) response_received = true;
                        if (current_state == DriveState::INIT_MAX_SPD && reg == 0x0033) response_received = true; 
                        if (current_state == DriveState::INIT_TIMEOUT && reg == 0x0039) response_received = true; 
                        if (current_state == DriveState::INIT_ACCEL && reg == 0x0031) response_received = true;
                        if (current_state == DriveState::INIT_DECEL && reg == 0x0032) response_received = true;
                        break;
                    }
                }
            }
    ...
    // Защита: Сброс автомата при потере связи (таймаут 2 секунды)
    ...
    if (response_received) {
        ...
        switch (current_state) {
            case DriveState::INIT_ENABLE:      current_state = DriveState::INIT_SUBDIVISION; break;
            case DriveState::INIT_SUBDIVISION: current_state = DriveState::INIT_START_SPD; break;
            case DriveState::INIT_START_SPD:   current_state = DriveState::INIT_MAX_SPD; break;
            case DriveState::INIT_MAX_SPD:     current_state = DriveState::INIT_ACCEL; break;
            case DriveState::INIT_TIMEOUT:     current_state = DriveState::INIT_ACCEL; break;
            case DriveState::INIT_ACCEL:       current_state = DriveState::INIT_DECEL; break;
            case DriveState::INIT_DECEL:
                ...
                break;
            case DriveState::RUN_WRITE_POS:     current_state = DriveState::RUN_READ_TELEMETRY; break;
            case DriveState::RUN_READ_TELEMETRY: current_state = DriveState::RUN_WRITE_POS; break;
        }
    }
    ...
    switch (current_state) {
        ...
        case DriveState::INIT_START_SPD:
            modbus_create_write_packet((uint8_t)slave_id.get(), 0x0030, 15, tx_packet); 
            _uart->write(tx_packet, 8);
            break;
            
        case DriveState::INIT_MAX_SPD:
            modbus_create_write_packet((uint8_t)slave_id.get(), 0x0033, 120, tx_packet); 
            _uart->write(tx_packet, 8);
            break;
    ...
    case DriveState::RUN_WRITE_POS: {
        ...
        // Жесткая константа в 20000 импульсов для настольной проверки
        int32_t target_pulses = (int32_t)(clean_steering * 20000.0);
