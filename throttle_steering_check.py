...
def modbus_udp_simulator():
    ...
    while True:
        try:
            ...
            # Отладочный вывод для шаговика руля
            print(f"[Modbus] ПОЛУЧЕНЫ БАЙТЫ ОТ ARDUPILOT: {req.hex().upper()} от {addr}")
            ...
            if func_code == 0x06:  # Блокировка обмоток шаговика
                ...
                if reg_addr == 0x0038 and val == 0x0001:
                    print("[Modbus Info] Обмотки рулевого привода заблокированы!")
            ...
            elif func_code == 0x03:  # Чтение ошибок CL57R
                if reg_addr == 0x0004:
